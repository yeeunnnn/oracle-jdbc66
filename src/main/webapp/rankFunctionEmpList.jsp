...
<%
	...
	/* 
		select 번호, 사원ID, 이름, 연봉, 급여순위
		from (select rownum 번호, 사원ID, 이름, 연봉, 급여순위
		      from (select employee_id 사원ID, last_name 이름, salary 연봉, rank() over(order by salary desc) 급여순위
		            from employees)); 
	*/
	int currentPage = 1;
	...
	// endRow가 10이 아닐때 남은 값을 출력하도록 lastPage 설정
	...
	String windowsSql = "select 번호, 사원ID, 이름, 연봉, 급여순위 from (select rownum 번호, 사원ID, 이름, 연봉, 급여순위 from (select employee_id 사원ID, last_name 이름, salary 연봉, rank() over(order by salary desc) 급여순위 from employees)) where 번호 between ? and ? ";
	...
	while(windowsRs.next()){
		...
		l.put("급여순위", windowsRs.getInt("급여순위"));
		list.add(l);
	}
...
<body>
	...
	<table border="1">
		...
		<%
			...
			// ex. 실제로는 23p인데 30p가 나오지 않도록 조건을 설정
